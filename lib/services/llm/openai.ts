...
export class OpenAITranslateService implements LLMTranslateService {
  public provider = 'openai' as const
    ...
    const tasks = texts.map((text, index) =>
      limit(async () => {
        try {
          ...
          const result = await pRetry(
            ...
            },
            {
              ...
              onFailedAttempt: (error) => {
                console.log(
                  `[OpenAI] Translation attempt ${error.attemptNumber} failed for segment ${index}. ${error.retriesLeft} retries left.`
                )
              },
            }
          )
          ...
          console.error(`[OpenAI] Failed to translate segment ${index}:`, error)
  ...
  async translateBatchMerged(
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
      ...
      try {
        ...
        console.error(`[OpenAI] Failed to translate batch ${batchIndex}:`, error)
